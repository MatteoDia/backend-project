                                            ...
                                            <div class="flex items-center">
                                                ...
                                                @else
                                                    <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center">
                                                        <span class="text-sm text-gray-500">{{ strtoupper(substr($user->name, 0, 1)) }}</span>
                                                    </div>
                                                @endif
                                                <div class="ml-4">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            @if(auth()->id() !== $user->id)
                                                <form action="{{ route('admin.users.toggle-admin', $user) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit" class="text-blue-600 hover:text-blue-900" onclick="return confirm('Are you sure you want to {{ $user->is_admin ? 'remove' : 'grant' }} admin privileges for this user?')">
